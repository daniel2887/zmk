...
set "EXTRA_FLAGS=-DEXTRA_CFLAGS=-ULV_CONF_PATH -DEXTRA_CXXFLAGS=-ULV_CONF_PATH"
...
for %%i in ("%SCRIPT_DIR%..\..\upstream\zmk-rgbled-widget") do set "RGB_MODULE=%%~fi"
set "RGB_MODULE=%RGB_MODULE:\=/%"
for %%i in ("%SCRIPT_DIR%..\zmk_driver_azoteq") do set "AZOTEQ_MODULE=%%~fi"
set "AZOTEQ_MODULE=%AZOTEQ_MODULE:\=/%"
for %%i in ("%SCRIPT_DIR%..\..\upstream\zmk-input-zoom") do set "ZOOM_MODULE=%%~fi"
set "ZOOM_MODULE=%ZOOM_MODULE:\=/%"
for %%i in ("%SCRIPT_DIR%..\zmk-pointing-acceleration") do set "ACCEL_MODULE=%%~fi"
...
set "LEFT_BUILD=west build --pristine=auto -b seeeduino_xiao_ble -d build/left -- %EXTRA_FLAGS% -DSHIELD="toucan_left rgbled_adapter nice_view_gem" -DZMK_CONFIG="%TOUCAN_CONFIG%" -DZMK_EXTRA_MODULES="%EXTRA_MODULES%""
set "RIGHT_BUILD=west build --pristine=auto -b seeeduino_xiao_ble -d build/right -- %EXTRA_FLAGS% -DSHIELD="toucan_right rgbled_adapter" -DZMK_CONFIG="%TOUCAN_CONFIG%" -DZMK_EXTRA_MODULES="%EXTRA_MODULES%""
